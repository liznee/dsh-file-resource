...
var ResourceService = class {
  ...
  async preview(sessionId, fileName) {
    ...
    if (match !== void 0) {
      const derived2 = await this.store.readDerivedForSession(sessionId, match.resourceId);
      return this.previewFrom(derived2, match);
    }
    const mention = (await this.store.listMentions(sessionId)).find((entry) => normalizeFileName2(entry.fileName) === want);
    if (mention === void 0) throw new Error("no attached file with that name in this session");
...
var ResourceStore = class {
  ...
  async readDerivedForResource(resourceId) {
    ...
    const parsed = JSON.parse((await gunzip(await readFile2(join(this.root, resource.derivedRelative)))).toString("utf8"));
