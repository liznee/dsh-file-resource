...
function inferPreferredResponseLanguage(messages) {
...
function languageContinuityPolicy(preferredLanguage = "auto") {
  return [
...
var init_language_policy = __esm({
  "src/language-policy.js"() {
    LANGUAGE_CONTINUITY_POLICY = languageContinuityPolicy();
  }
});
...
function createResourceRoute(service, {
  ...
  return {
    ...
    async handler(request, response) {
      ...
      try {
        const operation2 = header(request, "x-dsh-operation");
        const declaredLength = Number.parseInt(String(header(request, "content-length") ?? ""), 10);
        if (operation2 === "upload" && Number.isFinite(declaredLength) && declaredLength > maxFileBytes) {
          throw new RequestTooLargeError();
        }
        const sessionId = validateSessionId(header(request, "x-dsh-session"));
        if (operation2 === "upload" && request.method === "POST") {
        ...
        if (operation2 === "list" && request.method === "GET") {
          send(response, 200, { ok: true, resources: await service.listPending(sessionId) });
          return;
        }
        if (operation2 === "preview" && request.method === "GET") {
        ...
        if (operation2 === "remove" && request.method === "DELETE") {
        ...
        if ((operation2 === "commit" || operation2 === "wake") && request.method === "POST") {
        ...
        const message = operation === "preview" ? error instanceof Error ? error.message : "The file could not be processed." : "The file could not be processed.";
...
init_resource_reader();
function normalizeFileName2(name2) {
  return String(name2 ?? "").trim().replace(/^"|"$/gu, "");
}
var ResourceService = class {
  ...
  async preview(sessionId, fileName) {
    const want = normalizeFileName2(fileName);
    const resources = await this.store.listSession(sessionId);
    const match = resources.find((resource) => normalizeFileName2(resource.fileName) === want);
    if (match === void 0) throw new Error("no attached file with that name in this session");
    const derived = await this.store.readDerivedForSession(sessionId, match.resourceId);
    const selection = readSelection(derived, { selector: "summary", limit: 12e3 });
    return {
...
function createFileOnlyMessage(preferredLanguage = "auto") {
