...
async function openPreview({ document: documentRef = document, sessionId, fileName, t = (key) => key }) {
  ...
  const alignPanel = () => {
    const scroll = typeof documentRef.querySelector === "function" ? documentRef.querySelector("[data-conversation-scroll]") ?? documentRef.querySelector("[data-composer-card]") : null;
