...
test('parses UTF-8 and UTF-16 text locally without loading document parsers', async () => {
  let parserLoads = 0
  await withFile('notes.txt', Buffer.from('\ufeffhello 世界'), async resource => {
    const parsed = await parseResource({ ...resource, kind: 'text' }, {
      loadArchiveParser: async () => { parserLoads += 1; throw new Error('must not load') },
      loadPdfJs: async () => { parserLoads += 1; throw new Error('must not load') },
    })
...
test('normalizes archive chunks with page, slide and sheet selectors', async () => {
  const fakeArchive = {
    parseArchive: async () => ({
      kind: 'pptx',
      chunks: [
  ...
  await withFile('deck.pptx', Buffer.from('PK\u0003\u0004fixture'), async resource => {
    const parsed = await parseResource({ ...resource, kind: 'pptx' }, {
      loadArchiveParser: async () => fakeArchive,
    })
...
test('bounds extracted output to prevent decompression and parser amplification', async () => {
  const fakeArchive = {
    parseArchive: async () => ({ kind: 'docx', chunks: [{ text: 'x'.repeat(101), metadata: {} }] }),
  }
